...
impl AppConfig {
    ...
    pub fn new(
        ...
        Self::try_new(
            ...
            live2d_samples_dir, None, None, None,
        )
        .expect("valid CORS origin")
    }

    /// 10-arg constructor; returns a `Result` so `main` can render clean errors.
    #[allow(clippy::too_many_arguments)]
    pub fn try_new(
        port: Option<u16>, cors_origin: Option<String>, token_path: Option<PathBuf>,
        ledger_path: Option<PathBuf>, pet_root: Option<PathBuf>, memory_db: Option<PathBuf>,
...
mod tests {
    ...
    fn defaults() -> Result<AppConfig, ConfigError> {
        AppConfig::try_new(None, None, None, None, None, None, None, None, None, None)
    }
    ...
    fn project_root_defaults_to_cwd() {
        let cfg = AppConfig::try_new(
            ...
            Some(PathBuf::from("/tmp/xyz")), None, None,
        )
        .unwrap();
