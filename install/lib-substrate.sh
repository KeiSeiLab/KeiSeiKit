...
# Mirror pre-built substrate binaries from $KIT_DIR to $AGENTS_DIR.
...
copy_prebuilt_substrate_binaries() {
  ...
  local dst_dir="$AGENTS_DIR/_primitives/_rust/target/release"
  [ -d "$src_dir" ] || return 0
  mkdir -p "$dst_dir"
  ...
  if [ "$copied" -gt 0 ]; then
    say "  mirrored $copied substrate binar(y/ies) -> $dst_dir/"
