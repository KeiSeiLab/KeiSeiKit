...
menu_whiptail_profile() {
  ...
  "$tool" --title "KeiSeiKit Installer" --radiolist \
    "Choose install profile (SPACE to select, ENTER to confirm):" 24 78 9 \
    ...
    "cortex"   "kei-cortex daemon + cortex-ui browser app"          OFF \
...
menu_plain_profile() {
  ...
  echo "  7) cortex    — kei-cortex daemon + cortex-ui browser app"     >&2
  echo "  8) full      — all 36 primitives (~5 min, 200 MB)"            >&2
  echo "  9) custom    — pick individual primitives"                    >&2
  echo                                                                   >&2
  local reply
  printf 'Enter choice [1-9] (default 1): ' >&2
...
run_menu_if_needed() {
  ...
  if echo "$menu_out" | grep -q ','; then
    ...
    PROFILE="custom"
  elif echo "$menu_out" | grep -qE '^(minimal|core|frontend|ops|dev|mcp|cortex|full)$'; then
    PROFILE="$menu_out"
  else
