...
- **RULE 0.1** — no GitHub push (private remotes only).
- **RULE 0.11** — patent-portfolio DAG (FILING-SSOT.md + PATENT-GRAPH.md, both in same commit).
...
The `patent:`, `checkpoint:`, `feat:`, `refactor:`, `audit:`, and `fix:` commit prefixes are SSoT'd in [`~/.claude/rules/git-conventions.md`](https://example.invalid/git-conventions). `kei-changelog` walks Conventional Commits to regenerate `CHANGELOG.md` blocks; `kei-fork validate` checks the 6-file bundle on every merge.
